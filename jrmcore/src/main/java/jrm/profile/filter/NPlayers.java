...
/**
 * Manages the parsing, storage, and retrieval of player count configurations from an <code>nplayers.ini</code> metadata file.
 * <p>
 * This class maps games to their corresponding multiplayer mode tags (e.g., "2 Players", "4 Players") and implements
 * {@link jrm.profile.filter.IniProcessor} to filter and consume the file entries.
 * </p>
 * 
 * @author optyfr
 * 
...
public final class NPlayers implements Iterable<NPlayer>, IniProcessor {
    ...
    /**
     * Constructs a new {@code NPlayers} manager by parsing the provided {@code nplayers.ini} file.
     * 
     * @param file the configuration file containing multiplayer assignments
     * 
     * @throws IOException if the file cannot be read, or if no valid player data is extracted
     */
    ...
    /**
     * Factory method to read and instantiate a new {@code NPlayers} filter registry from the specified file.
     * 
     * @param file the target {@code nplayers.ini} file to parse
     * 
     * @return an initialized {@link NPlayers} manager containing mapped multiplayer categories
     * 
    ...
    /**
     * Returns an iterator over all parsed {@link NPlayer} multiplayer configuration categories.
     * 
