...
/**
 * Base abstract class representing a corrective action applied to a game/ROM
 * container (e.g., creating, opening, deleting, or torrent-zipping a ZIP/7Z
 * archive or a folder).
 * <p>
 * A {@code ContainerAction} maintains a list of individual {@link EntryAction}
 * operations that are executed on files nested inside the targeted container.
 * </p>
 * 
 * @author optyfr
 * @since 1.0
...
public abstract class ContainerAction implements StatusRendererFactory, Comparable<ContainerAction> {
    ...
    /**
     * The collection of nested structural entry actions to run inside this
     * container.
     */
    public final List<EntryAction> entryActions = new ArrayList<>();

    /**
     * Constructs a new {@code ContainerAction} for the specified container and
     * target format.
     * 
    ...
    /**
     * Performs the specific structural container action, using the given execution
     * session context and progress indicators.
     * 
    ...
    /**
     * Compares this action with another container action for task scheduling
     * priority, prioritizing smaller sizes and lower counts.
     * 
     * @param o the other container action to compare
     * @return a negative integer, zero, or a positive integer as this action is
     *         less than, equal to, or greater than the specified action
     */
    @Override
     ...
     * @return {@code true} if this object is equal to the obj argument, otherwise
     *         {@code false}
     */
    @Override
    ...
    /**
     * Returns a reverse descending priority sorting comparator (e.g., executing
     * largest tasks first).
     * 
    ...
    /**
     * Executes registered entry actions inside the provided generic compressed
     * archive.
     * 
     * @param session the current active session
     * @param handler the UI progress indicator
     * @param archive the target compressed archive interface
     * @return {@code true} if all child actions were executed successfully,
    ...
    /**
     * Executes registered entry actions inside the targeted zip file system
     * context.
     * 
     * @param session the current active session
     * @param handler the UI progress indicator
