...
public interface Archive extends Closeable, AutoCloseable {
    ...
    /**
     * Add an entry to the archive, The default assume that the {@code baseDir} is
     * from {@link #getTempDir()} then return
     * {@link #add(File baseDir, String entry)}
     * 
     * @param entry the entry to add, can contains sub directories, but always have
     *              to be a file
    ...
    /**
     * Add an entry to the archive, baseDir is the base directory of the entry and
     * should not be included in the archive path
     * 
     * @param baseDir the base directory
     * @param entry   the entry to add, can contains sub directories, but always
     *                have to be a file
     * @return 0 for success, anything else for error, -1 is reserved for read only
     *         archives
     * @throws IOException in case of IO error, can happen if entry not found or entry is a directory
    ...
    /**
     * Add an entry to the archive, named by entry, src data come from an
     * {@link InputStream}
     * 
    ...
    /**
     * Rename an entry into an archive. Be warned that, if using some external
     * commands in backend (like zip), renaming may not exists! So that, this
     * function will need to extract, rename, add, then delete!
     * 
     * @param entry   the entry to rename
     * @param newname the new name
     * @return 0 for success, anything else for error, -1 is reserved for read only
     *         archives
     * @throws IOException in case of IO error, can happen if new name already
     *                     exists
     */
