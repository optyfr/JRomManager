...
/**
 * Implementation of the StatusRenderer interface that generates HTML4-compliant output. This renderer provides methods to format text as HTML documents, apply styling such as bold and italic, and create progress bars using HTML tables. It also includes functionality to escape special characters for safe HTML rendering.
 * 
 * @author optyfr
 */
public class Html4Renderer implements StatusRenderer {

    /** Constructs a new Html4Renderer instance with default settings. This constructor does not perform any specific initialization and can be used to create a renderer for HTML4-compliant output. */
    public Html4Renderer() { /* default constructor */ }
    
    /** Converts the given character sequence into an HTML document format by wrapping it with the necessary HTML tags. This method takes a CharSequence as input and returns a String that represents the content formatted as an HTML document, including the &lt;html&gt; and &lt;body&gt; tags.
     * 
    ...
    /** Wraps the given character sequence in a &lt;nobr&gt; tag to prevent line breaks. This method takes a CharSequence as input and returns a String that represents the content wrapped in a &lt;nobr&gt; tag, ensuring that the text will not break into multiple lines when rendered in HTML.
     * 
    ...
    /** Converts the given character sequence into an HTML label format with optional styling. This method takes a CharSequence, a web color, and boolean flags for bold and italic styling. It returns a String that represents the input formatted as an HTML label, applying the specified color and styles as needed.
     * 
    ...
    /** Generates an HTML representation of a progress bar based on the given parameters. This method takes the width of the progress bar, the current progress value, the maximum value, and an optional message. It returns a String that represents the progress bar formatted as an HTML table, with the progress visually indicated by a colored cell and the message displayed alongside it if provided.
     * 
    ...
    /** Indicates whether this renderer supports progress rendering. In this implementation, it returns true, indicating that the Html4Renderer does support rendering progress bars using the progress method.
     * 
