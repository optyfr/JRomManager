 ...
 * </p>
 * <ul>
 * <li><b>Desktop (standalone):</b> A local session with an auto-generated
 * administrative user named {@code "JRomManager"}.</li>
 * <li><b>Single-user server:</b> A remote session identified by a unique
 * session identifier, without an explicitly assigned user.</li>
 * <li><b>Multi-user server:</b> A remote session identified by a unique
 * session identifier, with an explicitly assigned username and role set.</li>
 * </ul>
 *
 * <p>
 * Subclasses (such as web-server session variants) extend this class to add
 * transport-specific state, such as long-polling message queues and cached
 * resource lists.
 * </p>
 *
...
public class Session {
    ...
    /**
     * The unique session identifier string. This field is managed via a
     * Lombok-generated {@code setSessionId(String)} setter method and
     * accessed through the manual {@link #getSessionId()} getter method
     * defined in this class.
     *
     * @param sessionId the unique session identifier to set; may be
     *                  {@code null} for unassigned local sessions
     */
    private @Setter String sessionId;

    /**
     * The user context associated with this session. This field is declared
     * {@code protected} so that subclasses (e.g., web-server session variants)
     * and classes within the same package can access or override the user
     * reference directly. It may be {@code null} until a user is explicitly
     * assigned or lazily initialized via {@link #getUser()}.
     */
    ...
    /**
     * The session report accumulating scan findings, audit results, and
     * diagnostic entries. Initialized eagerly as an empty {@link Report} and
     * accumulates findings throughout the session's lifetime.
     *
     ...
     * @return the currently loaded ROM profile, or {@code null} if none is
     *         active
     */
     ...
     * framework-driven instantiation where session fields are populated
     * after construction.
     */
    protected Session() {
     ...
     * Initializes a default user named {@code "JRomManager"} with administrative
     * privileges and loads the default locale resource bundle.
     *
     * @param multiuser {@code true} to enable multi-user environment checks,
     *                  {@code false} for standard single-user
     * @param noupdate  {@code true} to disable check for updates, {@code false}
     *                  to permit updates
     */
    ...
    /**
     * Constructs a single-user server session with a specific session
     * identifier. The {@link #server} flag is set to {@code true} and the
     * default locale resource bundle is loaded. No explicit user is assigned;
     * one will be lazily created via {@link #getUser()} if accessed.
     *
    ...
    /**
     * Constructs a multi-user server session with a specific session
     * identifier, username, and role assignments. Both the {@link #server}
     * and {@link #multiuser} flags are set to {@code true}. If the username
     * or roles are {@code null}, sensible defaults are applied:
     * <ul>
     ...
     *                  {@code null}, defaults to {@code "server"}
     * @param roles     the roles assigned to the user; if {@code null},
     *                  defaults to a single {@code "admin"} role
     */
     ...
     * currently defined, a default administrator user named
     * {@code "JRomManager"} is automatically created and cached for
     * subsequent calls.
     *
     * @return the active {@link User} associated with this session; never
     *         {@code null}
     */
    public User getUser() {
        if (user == null)
     ...
     * Creates a new {@link User} instance with the given username and roles,
     * replacing any previously assigned user.
     *
