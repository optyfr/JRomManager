        ...
        return new TableCell<S, T>() {
            ...
            protected void updateItem(T item, boolean empty) {
                ...
                if (item == null || empty) {
                    ...
                    /*
                     * engine.documentProperty().addListener((obj, prev, newv) -> { String
                     * heightText = engine.executeScript( // <- Some modification, which gives
                     * moreless the same // result than the original "var body = document.body," +
                     * "html = document.documentElement;" +
                     * "Math.max( body.scrollHeight , body.offsetHeight, " +
                     * "html.clientHeight, html.scrollHeight , html.offsetHeight );").toString();
                     * Double height = Double.parseDouble(heightText.replace("px", "")) + 10; // <-
                     * Why are this 15.0 // required?? webview.setPrefHeight(height);
