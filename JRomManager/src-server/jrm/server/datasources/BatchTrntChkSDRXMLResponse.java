...
public class BatchTrntChkSDRXMLResponse extends XMLResponse
{
	...
	{
		List<SrcDstResult> sdrl =  SrcDstResult.fromJSON(request.session.getUser().settings.getProperty(SettingsEnum.trntchk_sdr, "[]"));
	...
	protected void add(Operation operation) throws Exception
	{
		...
		{
			final List<SrcDstResult> sdrl =  SrcDstResult.fromJSON(request.session.getUser().settings.getProperty(SettingsEnum.trntchk_sdr, "[]"));
			final SrcDstResult sdr = new SrcDstResult() {{src=new File(operation.getData("src"));}};
			if(!sdrl.contains(sdr))
			{
				sdrl.add(sdr);
				request.session.getUser().settings.setProperty(SettingsEnum.trntchk_sdr,SrcDstResult.toJSON(sdrl));
