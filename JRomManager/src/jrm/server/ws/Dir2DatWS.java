...
public class Dir2DatWS
{
	...
	void start(JsonObject jso)
	{
		(ws.session.worker = new Worker(() -> {
			...
			try
			{
				String srcdir = session.getUser().settings.getProperty("dir2dat_src_dir", null);
				String dstdat = session.getUser().settings.getProperty("dir2dat_dst_file", null);
				String format = session.getUser().settings.getProperty("dir2dat_format", "MAME");
